    ...
    mIdxInit(0), mScale(1.0), mInitSect(0), mbNotBA1(true), mbNotBA2(true), mIdxIteration(0), infoInertial(Eigen::MatrixXd::Zero(9,9))
{
    ...
    mbFirstGeoInitialized=false;
    mbSecondGeoInitialized=false;
...
void LocalMapping::SetImageMatcher(KeyPointImageMatcher *pImageMatcher)
{
    mpImageMatcher=pImageMatcher;
}

void LocalMapping::SendImgdata()
{

    NewImgdata Imgdata;
    Imgdata.sTimeStamp = mpCurrentKeyFrame->mTimeStamp;
    Imgdata.img = mpCurrentKeyFrame->mImRGB.clone();

    Imgdata.mnId = mpCurrentKeyFrame->mnId;
    // std::cout << "局部构图线程::图像尺寸:" << Imgdata.img.size << std::endl;
    // std::cout << "局部构图线程::图像ID:" << Imgdata.mnId << std::endl;
    if (!mbFirstGeoInitialized)
        Imgdata.bInitFinished = false;
    else
    {
        Imgdata.bInitFinished = true;
        Eigen::Matrix3d SRwg = mg2oSwg.rotation().toRotationMatrix();
        Eigen::Vector3d Stwg = mg2oSwg.translation();
        double Sswg = mg2oSwg.scale();

        Sophus::SE3d Tcw = mpCurrentKeyFrame->GetPose().cast<double>();
        Eigen::Matrix3d Rcw = Tcw.rotationMatrix();
        Eigen::Vector3d tcw = Tcw.translation();

        Eigen::Matrix3d Rcg = Rcw * SRwg;
        Eigen::Vector3d tcg = (Rcw * Stwg + tcw) / Sswg;
        Eigen::Matrix3d Rgc = Rcg.inverse();
        Eigen::Vector3d tgc = -Rgc * tcg;

        // 处理Rcg,使相机坐标系的z轴为正下视,这个操作在相机坐标系z轴朝下时通用
        Eigen::Matrix3d R_tmp, Rgc_tmp;
        R_tmp << 1, 0, 0,
            0, -1, 0,
            0, 0, -1;
        Rgc_tmp = R_tmp * Rgc;
        Eigen::AngleAxisd Vtmp;
        Vtmp = Rgc_tmp;
        Eigen::Vector3d Vcg_axis;
        Vcg_axis << 0, 0, Vtmp.axis()(2);
        // std::cout<<Vcg_axis<<std::endl;
        Eigen::AngleAxisd Vcg(Vtmp.angle(), Vcg_axis);
        Rgc = Vcg;
        Rgc = R_tmp.inverse() * Rgc;

        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                Imgdata.init_T_c2geo_data[i * 4 + j] = Rgc(i, j);
            }
            Imgdata.init_T_c2geo_data[i * 4 + 3] = tgc(i);
        }
        // std::cout<<"\n图像匹配线程::Rwc:\n"<<Rcw.inverse()<<std::endl;
        // std::cout<<"图像匹配线程::twc:"<<(-Rcw.inverse()*tcw).transpose()<<std::endl<<std::endl;
        // std::cout<<"图像匹配线程::发送的Rgc:\n"<<Rgc<<std::endl;
        // std::cout<<"图像匹配线程::发送的tgc:"<<tgc.transpose()<<std::endl<<std::endl;
    }

    mpImageMatcher->InsertImgdata(Imgdata);

    mpCurrentKeyFrame->mbImageMathed = true;
}

void LocalMapping::SetKeyFrameGeoPose()
{
    while (!mpImageMatcher->mlRefineImgdata.empty())
    {
        std::unique_lock<std::mutex> lock(mpImageMatcher->mMutexRefImgdata);
        RefineImgdata RefImgdata;
        RefImgdata = mpImageMatcher->mlRefineImgdata.front();
        mpImageMatcher->mlRefineImgdata.pop_front();

        vector<KeyFrame *> vpKFs = mpCurrentKeyFrame->GetMap()->GetAllKeyFrames();
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            KeyFrame *pKF = vpKFs[i];
            if (pKF->mbImageMathed&&!pKF->mbgeolocated&&pKF->mnId == RefImgdata.mnId)
            {
                Eigen::Matrix3d Rcgd;
                Eigen::Vector3d tcgd;
                for (int j = 0; j < 3; j++)
                {
                    for (int k = 0; k < 3; k++)
                    {
                        Rcgd(j, k) = RefImgdata.RefineT.at<double>(j, k);
                    }
                    tcgd(j) = RefImgdata.RefineT.at<double>(j, 3);
                }

                Eigen::JacobiSVD<Eigen::Matrix3d> svd(Rcgd, Eigen::ComputeFullU | Eigen::ComputeFullV);
                ...
                Eigen::Matrix3d Rcgd_tmp = U * S * V.transpose(); // 微调后的单位正交旋转矩阵

                Sophus::SE3d Tcgd(Rcgd_tmp, tcgd);
                pKF->mTcg = Tcgd.cast<float>();
                pKF->mbgeolocated = true;
                pKF->mnGeoId=RefImgdata.mnGeoId;
                pKF->mGeoInformation=RefImgdata.Information;
                
                std::cout<<"接受地理信息的关键帧id:"<<pKF->mnId<<std::endl<<std::endl<<std::endl;
                // std::cout<<"关键帧地理位姿：\n"<<pKF->mTcg.matrix()<<std::endl;
                break;
            }
        }
    }
...
void LocalMapping::FirstGeoInitialized()
{
    vector<KeyFrame *> vpKFs = mpCurrentKeyFrame->GetMap()->GetAllKeyFrames();
    KeyFrame *pFirstKF;
    KeyFrame *pSecondKF;
    int NumMaxKF = 1; // 选择第1帧和第NumMaxKF帧
    int NumKF = 0;
    
    // std::cout << "图像匹配线程::关键帧数量:"<<vpKFs.size()<< std::endl;

    for (size_t i = 0; i < vpKFs.size(); i++)
    {
        KeyFrame *pKF = vpKFs[i];
        // std::cout << "图像匹配线程" << pKF->mnId  << std::endl;
        if (pKF->mbgeolocated)
        {
            if (NumKF == 0)
            {
    ...
    //  std::cout << "图像匹配线程::关键帧数量:"<<NumKF<< std::endl;
    std::cout << "图像匹配线程::SLAM地理初始化关键帧1id:"<<pFirstKF->mnId<< std::endl;
    std::cout << "图像匹配线程::SLAM地理初始化关键帧2id:"<<pSecondKF->mnId<< std::endl;

    // 当前两个的关键帧成功进行图像匹配并赋了地理位姿时，开始SLAM初始化的第一步，初始化Sg2o::Sim3 mg2oSwg
    if (NumKF == NumMaxKF + 1)
    {
        std::cout << "\n局部构图线程::执行第一次地理初始化" << std::endl;
        Eigen::Matrix3d SRwg;
        Eigen::Vector3d Stwg;
        double Sswg;

        Sophus::SE3d FirstTcg = pFirstKF->mTcg.cast<double>();
        Sophus::SE3d SecondTcg = pSecondKF->mTcg.cast<double>();

        Sophus::SE3d FirstTgc = FirstTcg.inverse();
        Sophus::SE3d SecondTgc = SecondTcg.inverse();

        Sophus::SE3d FirstTwc = pFirstKF->GetPoseInverse().cast<double>();
        Sophus::SE3d SecondTwc = pSecondKF->GetPoseInverse().cast<double>();

        Sswg = (FirstTwc.translation() - SecondTwc.translation()).norm() / (FirstTgc.translation() - SecondTgc.translation()).norm();
        SRwg = FirstTwc.rotationMatrix() * FirstTcg.rotationMatrix();
        Stwg = Sswg * FirstTwc.rotationMatrix() * FirstTcg.translation() + FirstTwc.translation();
        Eigen::Quaterniond Sqwg(SRwg);
        Sqwg.normalize();
        g2o::Sim3 g2oSwg(Sqwg, Stwg, Sswg);

        // std::cout << "图像匹配线程::SLAM地理初始化FirstRwc:\n"
        //           << SecondTwc.rotationMatrix() << std::endl;
        // std::cout << "图像匹配线程::SLAM地理初始化Firsttwc:" << SecondTwc.translation().transpose() << std::endl;

        // std::cout << "图像匹配线程::SLAM地理初始化FirstRgc:\n"
        //           << SecondTgc.rotationMatrix() << std::endl;
        // std::cout << "图像匹配线程::SLAM地理初始化Firsttgc:" << SecondTgc.translation().transpose() << std::endl;

        Sophus::SE3d SecondTcw = SecondTwc.inverse();
        Eigen::Matrix3d Rcg = SecondTcw.rotationMatrix() * SRwg;
        Eigen::Vector3d tcg = (SecondTcw.rotationMatrix() * Stwg + SecondTcw.translation()) / Sswg;
        Eigen::Matrix3d Rgc = Rcg.inverse();
        Eigen::Vector3d tgc = -Rgc * tcg;

        // std::cout << "图像匹配线程::SLAM地理初始化验证的SecondRgc:\n"
        //           << Rgc << std::endl;
        // std::cout << "图像匹配线程::SLAM地理初始化验证的Secondtgc:" << tgc.transpose() << std::endl;

        // std::cout << "局部构图线程::SLAM地理初始化尺度:" << Sswg << std::endl;
        // std::cout << "局部构图线程::SLAM地理初始化位置:" << Stwg.transpose() << std::endl;
        // std::cout << "局部构图线程::SLAM地理初始化旋转矩阵:\n"
        //           << SRwg << std::endl;

        // Eigen::Matrix3d SRwg2;
        // Eigen::Vector3d Stwg2;
        // SRwg2 = SecondTwc.rotationMatrix() * SecondTcg.rotationMatrix();
        // Stwg2 = Sswg * SecondTwc.rotationMatrix() * SecondTcg.translation() + SecondTwc.translation();
        // Eigen::Quaterniond Sqwg2(SRwg2);
        // Sqwg2.normalize();
        // g2o::Sim3 g2oSwg2(Sqwg2, Stwg2, Sswg);

        mg2oSwg = g2oSwg;
        // std::cout << "局部构图线程::SLAM地理初始化:\n"
        //           << mg2oSwg << std::endl;
        // std::cout << "图像匹配线程::SLAM地理初始化2:\n"
        //           << g2oSwg2 << std::endl;

        std::cout << "局部构图线程::第一次SLAM地理初始化:\n"
                  << mg2oSwg << std::endl;
        std::cout << "局部构图线程::第一次SLAM地理结束!" << std::endl
                  << std::endl;
        mbFirstGeoInitialized = true;
    }
    return;
}

void LocalMapping::SecondGeoInitialized()
{
    vector<KeyFrame *> vpKFs = mpCurrentKeyFrame->GetMap()->GetAllKeyFrames();
    int NumKFGeolocated = 0;
    ...
    for (size_t i = 0; i < vpKFs.size(); i++)
    {
        ...
        {
            NumKFGeolocated++;
            // std::cout << "\n局部构图线程::第二次地理初始化输入关键帧的地理id:" << pKF->mnGeoId << std::endl;
        }
    }

    // 当地图中成功进行地理定位的关键帧数量达到10帧时，开启SLAM地理初始化第二阶段
    if (NumKFGeolocated >= 10)
    {
        std::cout << "\n局部构图线程::执行第二次地理初始化" << std::endl;
        int num_FixedKF_BA = 0;
        int num_OptKF_BA = 0;
        int num_MPs_BA = 0;
        int num_edges_BA = 0;
        g2o::Sim3 g2oSwg_tmp=mg2oSwg;
        
        // 这个地理BA函数具有地理初始化和局部BA两种模式
        Optimizer::GeoLocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, true, mg2oSwg, mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
        mbSecondGeoInitialized = true;
        mDoneBA = true;
         std::cout << "局部构图线程::第二次SLAM地理初始化前:\n"
                  << g2oSwg_tmp << std::endl;
        std::cout << "局部构图线程::第二次SLAM地理初始化后:\n"
                  << mg2oSwg << std::endl;
        std::cout << "局部构图线程::第二次SLAM地理结束!" << std::endl
                  << std::endl;
    }
    return;
}
...
void LocalMapping::Run()
{
    ...
    while (1)
    {
        ...
        if (CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            // 将关键帧的信息整理成Imgdata结构的形式，送入图像匹配线程
            if (mpImageMatcher->AcceptImgdata()&&!mbFirstGeoInitialized)
                SendImgdata();
#ifdef REGISTER_TIMES
            ...
            SetKeyFrameGeoPose(); // 进行局部地理优化前需要添加一个函数接收从图像匹配线程返回的无人机地理位姿

            if (!mbFirstGeoInitialized)
                FirstGeoInitialized();
            // else if (!mbSecondGeoInitialized)      
            //     SecondGeoInitialized();
            ...
            if (!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if (mpAtlas->KeyFramesInMap() > 2)
                {
                    ...
                    else
                    {
                        ...
                        if (!mDoneBA)
                        {
                            // if (mbSecondGeoInitialized)
                            //     Optimizer::GeoLocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, false, mg2oSwg, mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
                            // else
                ...
                if (mpCurrentKeyFrame->mbImageMathed == false)
                    KeyFrameCulling();
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // 该关键帧队列是Tracking线程向LocalMapping中插入的关键帧组成
    {
        ...
        std::cout << "局部构图线程::关键帧时间戳：" << mpCurrentKeyFrame->mTimeStamp << std::endl;
