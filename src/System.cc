    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // GetmDistCoef()
    // GetmK()
    // std::cout << "\n相机的畸变系数:\n"<< mpTracker->GetmDistCoef() <<"\n相机内参矩阵:\n"<<mpTracker->GetmK() <<std::endl;

    // 开启图像匹配线程
    mpImageMatcher = new KeyPointImageMatcher(mpTracker->GetmK(), mpTracker->GetmDistCoef(),init_T_c2geo_tq);
    mptImageMatcher = new thread(&KeyPointImageMatcher::Run, mpImageMatcher);
    // 使得局部构图线程和跟踪线程能访问图像匹配线程
    mpTracker->SetImageMatcher(mpImageMatcher);
    mpLocalMapper->SetImageMatcher(mpImageMatcher);
...
void System::SaveTrajectoryGeo(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        else
        {
            ...
            g2o::Sim3 Swg = mpLocalMapper->mg2oSwg;
            g2o::Sim3 Sgw = Swg.inverse();
            g2o::Sim3 Swc(Twc.unit_quaternion().cast<double>(), Twc.translation().cast<double>(), 1);
            ...
            g2o::Sim3 Scc(r_tmp, t_tmp, Swg.scale());
            g2o::Sim3 Sgc=Sgw*Swc*Scc;

            Eigen::Quaterniond q = Sgc.rotation();
            Eigen::Vector3d tgc = Sgc.translation();

            f << setprecision(6) << (*lT) << " " <<  setprecision(9) << tgc(0)+init_T_c2geo_tq[0] << " " << tgc(1)+ init_T_c2geo_tq[1]<< " " << tgc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
    ...
    //cout << "end saving trajectory" << endl;
    f.close();
    cout << endl << "End of saving geographical trajectory to " << filename << " ..." << endl;
}

void System::SaveTrajectoryGeoz0(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            ...
            Eigen::Quaternionf q = Twb.unit_quaternion();
            Eigen::Vector3f twb = Twb.translation();
            f << setprecision(6) << 1e9*(*lT) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " 0 " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
        else
        {
            Sophus::SE3f Twc = ((*lit)*Trw).inverse();
            ...
            f << setprecision(6) << (*lT) << " " <<  setprecision(9) << tgc(0)+init_T_c2geo_tq[0] << " " << tgc(1)+ init_T_c2geo_tq[1] << " 0 " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
