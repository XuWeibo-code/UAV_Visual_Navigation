...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        if (pKFi->mnId == pMap->GetInitKFid())
            std::cout << "第" << pKFi->mnId << "个关键帧被固定！" << std::endl;
        optimizer.addVertex(vSE3);
        if (pKFi->mnId > maxKFid)
            maxKFid = pKFi->mnId;
        // DEBUG LBA
...
void Optimizer::GeoLocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, bool GeoInitialize, g2o::Sim3& g2oSwg, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
{
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame *> lLocalKeyFrames;
    ...
    // Fixed Keyframes.  Keyframes that see Local MapPointsbut that are not Local Keyframes
    list<KeyFrame*> lFixedCameras;
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        map<KeyFrame*,tuple<int,int>> observations = (*lit)->GetObservations();
    ...
    // std::cout<<"地理BA函数::构建优化器，开始优化！"<<std::endl;
    ...
    optimizer.setVerbose(false);
    // if (GeoInitialize)
    //     solver->setUserLambdaInit(0.1);

    // SLAM的地理初始化很重要，不可中断
    if (pbStopFlag && !GeoInitialize)
        optimizer.setForceStopFlag(pbStopFlag);
    ...
    // std::cout << "地理BA函数::插入关键帧顶点！" << std::endl;
    // Set Local KeyFrame vertices
    for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
    {
        KeyFrame *pKFi = *lit;
    ...
    // Set MapPoint vertices
    const int nExpectedSize = (lLocalKeyFrames.size() + lFixedCameras.size()) * lLocalMapPoints.size();
    const int nExpectedGeoSize = lLocalKeyFrames.size();

    // 存储SLAM重投影边
    vector<ORB_SLAM3::EdgeSE3ProjectXYZ *> vpEdgesMono;
    vpEdgesMono.reserve(nExpectedSize);
    //
    vector<g2o::EdgeSim3InGeoSLAM *> vpEdgesGeo;
    vpEdgesGeo.reserve(nExpectedGeoSize);

    vector<KeyFrame *> vpEdgeKFMono;
    vpEdgeKFMono.reserve(nExpectedSize);

    vector<KeyFrame *> vpEdgeKFGeo;
    vpEdgeKFGeo.reserve(nExpectedGeoSize);
    ...
    // std::cout << "地理BA函数::插入地图点顶点和投影边！" << std::endl;
    ...
    // std::cout << "地理BA函数::插入Sgw顶点！" << std::endl;
    // 增加Sgw的顶点
    g2o::VertexSim3Expmap *vg2oSgw = new g2o::VertexSim3Expmap();

    vg2oSgw->setEstimate(g2oSwg.inverse());
    vg2oSgw->setId(MaxMPid + 1);
    // std::cout << "地理BA函数::Sgw顶点id:" << (MaxMPid + 1) << std::endl;
    if (!GeoInitialize)
        vg2oSgw->setFixed(true);

    optimizer.addVertex(vg2oSgw);
    // std::cout << "地理BA函数::插入地理边！" << std::endl;
    int num_geo_edge = 0;
    for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
    {
        KeyFrame *pKFi = *lit;
        if (pKFi->mbgeolocated)
        {
            if (pKFi->mnId == pMap->GetInitKFid() && !GeoInitialize)
                continue;

            g2o::EdgeSim3InGeoSLAM *e = new g2o::EdgeSim3InGeoSLAM();
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(MaxMPid + 1)));
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKFi->mnId)));

            Sophus::SE3<float> Tcg = pKFi->mTcg;
            g2o::Sim3 obs(Tcg.unit_quaternion().cast<double>(), Tcg.translation().cast<double>(), 1);
            ...
            I7(2, 2) = 20000;
            // I7(5, 5) = 0.01;
            // I7 = pKFi->mGeoInformation * I7;

            if (GeoInitialize)
                e->setInformation(I7 * 600);
            else
                e->setInformation(I7 * 200);
            // 图像匹配的鲁棒和函数难以控制
             g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
             e->setRobustKernel(rk);
            //  rk->setDelta(1000);
            optimizer.addEdge(e);
            vpEdgesGeo.push_back(e);
            vpEdgeKFGeo.push_back(pKFi);
            num_geo_edge++;
        }
    }
    //  std::cout << "地理BA函数::插入" <<num_geo_edge<<"个地理边" <<std::endl;

    num_edges = nEdges;

    if (pbStopFlag && !GeoInitialize) // 地理初始化时停不得
        if (*pbStopFlag)
            return;
    // std::cout << "地理BA函数::开始优化！" << std::endl;
    optimizer.initializeOptimization();
    optimizer.optimize(10);

    double Error_Mono = 0, Error_Geo = 0;
    // 输出投影边和地理边的调试信息
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
    {
        ORB_SLAM3::EdgeSE3ProjectXYZ *e = vpEdgesMono[i];
    ...
    for (size_t i = 0, iend = vpEdgesGeo.size(); i < iend; i++)
    {
        g2o::EdgeSim3InGeoSLAM *e = vpEdgesGeo[i];
        KeyFrame *pKFi = vpEdgeKFGeo[i];

        Error_Geo += e->chi2();
        std::cout << "地理BA函数::第" << pKFi->mnId << "个关键帧对应的地理边误差:" << e->chi2() << std::endl;
         if (e->chi2() > 1000)
            pKFi->mGeoInformation = pKFi->mGeoInformation * 0.8;
       

        // std::cout << "地理BA函数::第" << i << "个地理边误差:" << e->error().transpose() << std::endl;
    }

    std::cout << std::endl;
    std::cout << "\n地理BA函数::优化信息输出：" << std::endl;
    std::cout << "地理BA函数::优化关键帧数:" << lLocalKeyFrames.size() << std::endl;
    std::cout << "地理BA函数::优化投影边数:" << vpEdgesMono.size() << std::endl;
    std::cout << "地理BA函数::优化地理边数:" << vpEdgesGeo.size() << std::endl;
    std::cout << "地理BA函数::重投影误差:" << Error_Mono << std::endl;
    std::cout << "地理BA函数::地理误差:" << Error_Geo << std::endl;
    std::cout << "地理BA函数::误差比:" << Error_Mono / Error_Geo << std::endl;
    std::cout << std::endl;

    // std::cout << "地理BA函数::恢复优化数据！" << std::endl;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
     ...
     if (GeoInitialize)
    {
        g2o::VertexSim3Expmap *vnewg2oSgw = static_cast<g2o::VertexSim3Expmap *>(optimizer.vertex(MaxMPid + 1));
        g2oSwg = vnewg2oSgw->estimate().inverse();
        // std::cout << "优化程序::优化后的g2oSwg\n"
        //           << g2oSwg << std::endl;
    }
