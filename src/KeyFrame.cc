    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
{
    ...
    mbImageMathed=false;//构建关键帧时未进行图相匹配
    mbgeolocated=false;//构建关键帧时未进行地理定位

    // std::cout<<"构建关键帧时::帧图像尺寸："<<F.mImRGB.size()<<std::endl;
    // std::cout<<"构建关键帧时::关键帧图像尺寸："<<mImRGB.size()<<std::endl;
    mnId = nNextId++;
...
{
    {
        unique_lock<mutex> lock(mMutexConnections);
        // 初始关键帧不能删除
        if (mnId == mpMap->GetInitKFid()||mbImageMathed==true)
        {
            return;
        }
        else if (mbNotErase)
