...
void Tracking::SetImageMatcher(KeyPointImageMatcher *pImageMatcher)
{
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::MONOCULAR)
    {
        if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET || (lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray, ImRGB, timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth);
        else
            mCurrentFrame = Frame(mImGray, ImRGB, timestamp, mpORBextractorLeft, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth);
    }
    else if (mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            mCurrentFrame = Frame(mImGray, ImRGB, timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, &mLastFrame, *mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray, ImRGB, timestamp, mpORBextractorLeft, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, &mLastFrame, *mpImuCalib);
    }
    // std::cout << "跟踪线程::图像尺寸:" << mCurrentFrame.mImRGB.size << std::endl;
    // t0存储未初始化时的第1帧图像时间戳
...
// std::cout<<"初始化构建关键帧时线程::图像1尺寸："<<pKFini->mImRGB.size()<<std::endl;
// std::cout<<"初始化构建关键帧时线程::图像2尺寸："<<pKFcur->mImRGB.size()<<std::endl;
